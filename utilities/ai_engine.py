...
class AIAutomationFramework:
    ...
    def highlight_element(self, loc):
        """RESTORED: Visual Audit - Highlights the discovered element."""
        try:
    ...
    def _find_locator_weighted(self, intent):
        ...
        for match in matches[:3]:
            ...
            if el['placeholder']:
                candidates.append({"strategy": "css_selector", "value": f"[placeholder='{el['placeholder']}']"})
            ...
            for cand in candidates:
                if self._is_unique(cand):
                    self.highlight_element(cand)  # Trigger highlight
                    return cand
        return None
    ...
    def get_step_metadata(self, step_text):
        ...
        for raw_val in raw_values:
            ...
            if loc:
                ...
                results.append({
                    "intent": intent, "action": action, "tag": tag,
                    "role": role, "locator": loc, "test_data": raw_val
                })
            ...
            if click_match:
                ...
                if loc:
                    ...
                    results.append({
                        ...
                        "tag": matched_el.get('tag', 'button'), "role": matched_el.get('role', ''),
                        "locator": loc, "test_data": None
                    })

        # RESTORED: Audit Screenshot
        if results:
